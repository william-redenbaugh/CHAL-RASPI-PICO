...
#if DEBUG > 0
#define PRT_DEBUG(a, ...)                                                                                                               \
    printf(DEBUG_COLOR "%s [%d]:%5d:%5d ", logTimeBuf, get_core_num(), xPortGetFreeHeapSize(), (int)uxTaskGetStackHighWaterMark(NULL)); \
    printf((a), ##__VA_ARGS__)
#define ALERT_DEBUG(a, ...)                                                                                                                                    \
    printf(_ALERT_RED_ "%s [%d]:%5d:%5d "_ALERT_OFF_ DEBUG_COLOR, logTimeBuf, get_core_num(), xPortGetFreeHeapSize(), (int)uxTaskGetStackHighWaterMark(NULL)); \
    printf((a), ##__VA_ARGS__)
...
#if DEBUG > 2
#define TRC_DEBUG(a, ...)                                                                                          \
    printf(DEBUG_COLOR "%s %5d:%5d ", logTimeBuf, xPortGetFreeHeapSize(), (int)uxTaskGetStackHighWaterMark(NULL)); \
    printf((a), ##__VA_ARGS__)
#else
